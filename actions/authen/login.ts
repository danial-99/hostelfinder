...
export async function Login(formData: FormData) {
  try {
    ...
    if (!existingUser) {
      return { status: 404, success: false, message: "User not found", error: "User not found" };
    }
    ...
    if (!passwordMatch) {
      return { status: 401, success: false, message: "Invalid credentials", error: "Invalid credentials" };
    }
